 ...
 */
 ...
 /* Calculate direction of gravity indicated by measurement */
  ...
  Quat->Deviate[2] = 0.f;
	...
	/* Update the Euler angle in radians */
	float pitch_val = 2.f*(Quat->quat[0]*Quat->quat[2]) - 2.f*(Quat->quat[1]*Quat->quat[3]);

	// 安全钳位，防止 asinf 产生 NaN
	...
	Quat->EulerAngle[1] = asinf(pitch_val); // 使用钳位后的值
...
static void QuaternionEKF_xhat_Update(KalmanFilter_Info_TypeDef *KF){
  ...
  KF->Data.Cache_Vector[0][2] =   KF->Data.Xhatminus[0] * KF->Data.Xhatminus[0] \
	...
	{
		float gravity_val = fabsf(KF->Data.Cache_Vector[0][i]);

		// 安全钳位，防止 acosf 产生 NaN
		if(gravity_val > 1.0f) gravity_val = 1.0f;

		OrientationCosine[i] = acosf(gravity_val);
	}
